...
use bitview_compute::{
    BlockCountTarget24h, CACHE_BUDGET, CachedBlockCountReader, CachedWindowStartVec, ConstantVecs,
    Identity, LazyPerBlock, LazyPercentCumulativeRolling, LazyPercentPerBlock, LazyWindowStartVec,
    PerBlock, RatioU64, WindowId, Windows,
};
use bitview_traversable::Traversable;
use brk_types::{Height, PartsPerMillion32, StoredU16, StoredU64, Timestamp, Version};
use vecdb::{
    AnyStoredVec, AnyVec, BinaryTransform, CachedVec, ColumnId, Database, IndexVec, LazyVec,
...
fn views_recompute_cheap_outputs_and_share_only_their_roots() {
    ...
    indexes.cached_first_height.day1.snapshot();
    ...
    let converted = LazyPerBlock::from_height_source::<Identity<StoredU64>>(
        "converted",
        Version::ONE,
        &view,
        &indexes,
    );
    let alias = LazyPerBlock::from_lazy::<Identity<StoredU64>, StoredU64>(
        "alias",
        Version::ONE,
        &converted,
    );
    ...
    let mut metric =
        PerBlock::<StoredU64>::forced_import(&db, "stored_metric", Version::ONE, &indexes).unwrap();
    for i in 0..N {
    ...
    let compact = CachedBlockCountReader::new(counts.read_only_cached_boxed_clone());
    ...
    let constants =
        ConstantVecs::new::<BlockCountTarget24h>("target", Version::ONE, &constant_indexes);
    vecdb::diagnostics::take();
