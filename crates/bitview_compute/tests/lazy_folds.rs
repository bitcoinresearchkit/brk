...
use bitview_compute::{
    CachedBlockCountReader, LazyIndexCountVec, LazyLookbackVec, LazyPreviousDeltaVec,
    LazyRatioWithCachedBlockCount, LazyRollingRatioVec, LazyRollingRatioWithCachedBlockCount,
    LazySinceDayVec, LazyWindowVec, RatioU64,
};
use brk_types::{Day1, Height, PartsPerMillion32, StoredU16, StoredU64, Version};
use vecdb::{
    AnyStoredVec, CachedVec, Database, EagerVec, ImportableVec, PcoVec, PcoVecValue, ReadBounds,
    ReadableCloneableVec, ReadableVec, VecValue, WritableVec,
};
...
fn folds_match_materialization_for_all_optimized_views_and_published_bounds() {
    ...
    let denominator = CachedBlockCountReader::new(denominator.read_only_cached_boxed_clone());
