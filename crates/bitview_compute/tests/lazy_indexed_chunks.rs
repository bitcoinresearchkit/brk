...
use bitview_compute::{Identity, LazyIndexedVec};
use brk_types::{Height, StoredU64, Version};
use tempfile::tempdir;
use vecdb::{
    AnyStoredVec, BytesVec, CachedReadableVec, CachedVec, Database, ImportableVec, LazyVec,
    MutableVec, ReadOnlyClone, ReadableCloneableVec, ReadableVec, WritableVec,
};
...
fn indexed_chunks_preserve_captures_offsets_short_metadata_and_rewrites() {
    ...
    let indexed = LazyIndexedVec::new(
        ...
        cached_metadata.cached_boxed_clone(),
    ...
    let identity = LazyVec::<Height, StoredU64, Height, StoredU64>::transformed::<
        Identity<StoredU64>,
    >("identity", Version::ONE, indexed.read_only_boxed_clone());
    for (from, to) in [
        (0, 40_000),
        (16_000, 33_000),
        (34_990, 40_000),
...
fn indexed_chunks_keep_metadata_aligned_with_emitted_values_across_holes() {
    ...
    let indexed = LazyIndexedVec::new(
        ...
        Version::ONE,
        source.read_only_boxed_clone(),
        CachedVec::wrap(metadata).read_only_cached_boxed_clone(),
