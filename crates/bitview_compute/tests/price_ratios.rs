...
use bitview_compute::{
    CACHE_BUDGET, ColumnarPerBlock, IndexSources, LazyColumnPriceWithRatioPerBlock, LazyIndexedVec,
    LazyPriceWithRatioPerBlock, LazyRatioPerBlock, PriceWithRatioPerBlock, WindowId,
};
...
fn cached_price_constructors_preserve_ratio_sources_names_and_versions() {
    ...
    indexes.cached_first_height.day1 = CachedVec::wrap(stored(
        &db,
        "ratio_days",
    ...
    macro_rules! check {
        ($view:ident, $name:literal) => {{
            ...
            assert!($view.ppm.height.collect_one_at(0).unwrap().is_nan());
            let TreeNode::Leaf(leaf) = $view.ppm.to_tree_node() else {
            ...
            assert_eq!($view.ppm.height.collect_one_at(4), Some(PriceRatio::MAX));
            assert!($view.ppm.height.collect_one_at(5).unwrap().is_nan());
            assert_eq!($view.cents.height.collect_one_at(4), Some(Cents::new(1)));
            assert_eq!(
                f32::from($view.ratio.height.collect_one_at(4).unwrap()),
                f32::from(PriceRatio::MAX)
            );
            assert!(f32::from($view.ratio.height.collect_one_at(5).unwrap()).is_nan());
            assert_eq!(
                $view.ppm.height.collect_range_at(1, 4),
                [4.0, 3.0, 2.0].map(PriceRatio::from)
            );
            assert_eq!($view.ppm.height.name(), expected.ppm.height.name());
            assert_eq!($view.ppm.height.version(), expected.ppm.height.version());
            assert_eq!($view.ppm.day1.collect(), expected.ppm.day1.collect());
            assert_eq!(
                $view.ratio.height.collect_range_at(1, 4),
                expected.ratio.height.collect_range_at(1, 4)
            );
            assert_eq!($view.ratio.height.name(), expected.ratio.height.name());
            assert_eq!(
                $view.ratio.height.version(),
