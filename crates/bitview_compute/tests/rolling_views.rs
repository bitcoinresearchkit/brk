...
fn rolling_units_preserve_height_and_all_resolution_views() {
    ...
    macro_rules! mappings {
        ($($field:ident),+ $(,)?) => {$(
            indexes.cached_first_height.$field = CachedVec::wrap(stored(
    ...
    indexes.cached_first_height.halving =
        CachedVec::wrap(stored(&db, "populated_halving", [Height::ZERO]))
            .read_only_cached_boxed_clone();
    indexes.cached_first_height.epoch =
        CachedVec::wrap(stored(&db, "populated_epoch", [Height::ZERO]))
            .read_only_cached_boxed_clone();
...
fn compact_and_general_sum_constructors_match() {
    ...
    let compact = LazyRollingSumsFromHeight::from_compact_cumulative(
        "counts_sum",
        Version::ONE,
        &cumulative,
        &starts,
