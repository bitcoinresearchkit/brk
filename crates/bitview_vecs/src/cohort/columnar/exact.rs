...
use bitview_cohort::{
    ...
    UNDER_AMOUNT_FILTERS, UTXO_AGGREGATE_FILTERS, UTXOAggregateId, UTXOAggregateRows, UTXORows,
    UnderAgeId, UnderAmountId,
};
...
use vecdb::{
    ...
    Database, EagerVec, ImportableVec, PcoVec, PcoVecValue, ReadOnlyClone, ReadableColumnarVec, Rw,
    StorageMode, WritableVec,
};

use super::UTXOColumnarMetric;
    ...
    T: PcoVecValue + AddAssign,
{
    ...
    pub fn source(
        ...
        if let Some(source) = self.direct.direct_source(cache, filter, name, version) {
            return Some(source);
        }
        ...
        match filter {
            Filter::All | Filter::Term(_) => UTXOAggregateId::ALL
                ...
                .map(|id| Self::column(cache, &self.aggregate_matrix, name, version, id)),
            Filter::Time(_) => UnderAgeId::ALL
                .iter()
                .copied()
                .find(|id| id.select(&UNDER_AGE_FILTERS) == filter)
                .map(|id| Self::column(cache, &self.under_age_matrix, name, version, id))
                .or_else(|| {
                    OverAgeId::ALL
                        .iter()
                        .copied()
                        .find(|id| id.select(&OVER_AGE_FILTERS) == filter)
                        .map(|id| Self::column(cache, &self.over_age_matrix, name, version, id))
                }),
            Filter::Amount(_) => UnderAmountId::ALL
                .iter()
                .copied()
                .find(|id| id.select(&UNDER_AMOUNT_FILTERS) == filter)
                .map(|id| Self::column(cache, &self.under_amount_matrix, name, version, id))
                .or_else(|| {
                    OverAmountId::ALL
                        .iter()
                        .copied()
                        .find(|id| id.select(&OVER_AMOUNT_FILTERS) == filter)
                        .map(|id| Self::column(cache, &self.over_amount_matrix, name, version, id))
                }),
