...
fn additive_and_cumulative_sources_share_exact_aggregate_selection() {
    ...
    let mut additive =
        UTXOCoreColumns::<Sats>::forced_import(cache, &db, "additive", Version::ONE).unwrap();
    let mut cumulative =
        CumulativeUTXOCoreValueColumns::forced_import(cache, &db, "cumulative", Version::ONE)
            .unwrap();
    let mut full =
        CumulativeUTXOValueColumns::forced_import(cache, &db, "full", Version::ONE).unwrap();
    let sats = UTXORows {
        core: UTXOCoreRows {
