...
use vecdb::{CachedVec, Database, Ident, ReadOnlyClone, ReadableVec};
...
fn resolution_families_preserve_order_empty_buckets_and_both_derived_constructors() {
    ...
    macro_rules! initialize_mappings {
        (
            ...
            $(indexes.first_height.$field = CachedVec::wrap(common::stored(
                &db, stringify!($field), [0_usize, 2, 2].map(Height::from),
            )).read_only_boxed_clone();)*
            $(indexes.first_height.$epoch = CachedVec::wrap(common::stored(
                &db, stringify!($epoch), [Height::ZERO],
            )).read_only_boxed_clone();)*
        };
    }
    bitview_collections::with_resolution_fields!(initialize_mappings);
    ...
    let direct =
        DerivedResolutions::from_derived_computed::<Ident>("metric", Version::ONE, &resolutions);
    let chained =
        DerivedResolutions::from_lazy::<Ident, StoredU64>("metric", Version::ONE, &direct);
    ...
    bitview_collections::with_resolution_fields!(check_values);
    assert_eq!(
