use brk_types::{Bitcoin, Close, Dollars, Sats, StoredF32, StoredF64, StoredU32, StoredU64, Weight};
...
impl BinaryTransform<StoredF32, StoredF32, StoredF32> for RsiFormula {
    #[inline(always)]
    fn apply(avg_gain: StoredF32, avg_loss: StoredF32) -> StoredF32 {
        let sum = *avg_gain + *avg_loss;
        if sum == 0.0 {
            StoredF32::from(50.0)
        } else {
            StoredF32::from(100.0 * *avg_gain / sum)
        }
    }
}
