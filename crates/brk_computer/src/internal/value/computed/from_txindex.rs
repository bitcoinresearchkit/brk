...
use crate::{ComputeIndexes, internal::Source, indexes, price, utils::OptionExt};

use crate::internal::{ComputedVecsFromTxindex, VecBuilderOptions};
...
pub struct ComputedValueVecsFromTxindex {
    ...
    // Derives directly from sats source (Eager) to avoid Lazy <- Lazy
...
impl ComputedValueVecsFromTxindex {
    ...
    pub fn forced_import(
        ...
        // Compute sats source once - used by both bitcoin_txindex and dollars_txindex
        let sats_source = source
            .vec()
            .unwrap_or_else(|| sats.txindex.u().boxed_clone());
        ...
        // Derive directly from sats source (Eager) to avoid Lazy <- Lazy
        let dollars_txindex = price.map(|price| {
            LazyVecFrom3::init(
                ...
                |txindex: TxIndex,
                 txindex_to_sats_iter,
                 txindex_to_height_iter,
                 height_to_price_close_iter| {
                    let txindex = txindex.to_usize();
                    txindex_to_sats_iter.get_at(txindex).and_then(|sats| {
