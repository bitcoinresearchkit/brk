...
use super::Vecs;
use crate::internal::{
    LazyFromDateLast, RatioF32, StoredF32TimesSqrt7, StoredF32TimesSqrt30, StoredF32TimesSqrt365,
};
...
impl Vecs {
    pub fn forced_import(version: Version, returns: &returns::Vecs) -> Self {
        ...
        let price_1w_volatility = LazyFromDateLast::from_source::<StoredF32TimesSqrt7>(
            "price_1w_volatility",
            version + v2,
            &returns._1d_returns_1w_sd.sd,
        );

        let price_1m_volatility = LazyFromDateLast::from_source::<StoredF32TimesSqrt30>(
            "price_1m_volatility",
            version + v2,
            &returns._1d_returns_1m_sd.sd,
        );

        let price_1y_volatility = LazyFromDateLast::from_source::<StoredF32TimesSqrt365>(
            "price_1y_volatility",
            version + v2,
