...
use super::Vecs;
use crate::internal::{HeightDateBytes, HeightDateLazyOHLC, LazyOHLC};

impl Vecs {
    pub fn forced_import(db: &Database, version: Version) -> Result<Self> {
        let ohlc: HeightDateBytes<OHLCCents> =
            HeightDateBytes::forced_import(db, "ohlc_cents", version)?;

        let components = HeightDateLazyOHLC {
            height: LazyOHLC {
