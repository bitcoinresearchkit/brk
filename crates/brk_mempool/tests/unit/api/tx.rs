...
fn aggregates_require_completed_observations_including_empty_results() {
    ...
    let reader = mempool.read_only_clone();
    let check = |ready| {
        assert_eq!(reader.load().txids_hash().is_ok(), ready);
        assert_eq!(reader.load().txids_with_hash().is_ok(), ready);
        assert_eq!(reader.load().recent_txs().is_ok(), ready);
        assert_eq!(
            reader.load().transaction_times_with_hash(&[txid]).is_ok(),
            ready
        );
    };
    check(false);
    ...
    check(true);
    assert!(mempool.published().txids_with_hash().unwrap().0.is_empty());
    assert_eq!(
        mempool
            .published()
            .transaction_times_with_hash(&[txid])
            .unwrap()
            .0,
        vec![0]
    );
    ...
    check(true);
    let (txids, hash) = mempool.published().txids_with_hash().unwrap();
    assert_eq!(txids, vec![txid]);
    assert_eq!(hash, mempool.published().txids_hash().unwrap());
}
