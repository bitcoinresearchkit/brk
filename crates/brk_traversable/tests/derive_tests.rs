...
// ============================================================================
// When same metric names used across denominations, merge collapses them

#[derive(Traversable)]
#[traversable(merge)]
struct MockValueBlockFullMerged {
    pub sats: MockComputedBlockFull,
    pub bitcoin: MockComputedBlockFull,
    pub dollars: Option<MockComputedBlockFull>,
}
...
#[test]
fn value_block_full_merged_same_names_collapse() {
    // Using SAME metric names across all denominations
    let value = MockValueBlockFullMerged {
        sats: make_computed_block_full("metric"),
        bitcoin: make_computed_block_full("metric"),  // Same name!
        dollars: Some(make_computed_block_full("metric")),  // Same name!
    };

    let tree = value.to_tree_node();

    // With merge and same metric names, all leaves collapse
    // Result: flat tree with each metric type having all denomination indexes merged
    match &tree {
        TreeNode::Branch(map) => {
            // Should be flat - no sats/bitcoin/dollars branches
            assert!(!map.contains_key("sats"), "sats branch should be lifted");
            assert!(!map.contains_key("bitcoin"), "bitcoin branch should be lifted");
            assert!(!map.contains_key("dollars"), "dollars branch should be lifted");

            // Should have metric keys: base, cumulative, dateindex, difficultyepoch
            assert!(map.contains_key("base"), "Missing base key");
            assert!(map.contains_key("cumulative"), "Missing cumulative key");

            // base should have all Height indexes merged (same metric name = collapsed)
            if let Some(TreeNode::Leaf(l)) = map.get("base") {
                assert_eq!(l.name(), "metric");
                // All 3 denominations have Height index
                let indexes = l.indexes();
                assert!(indexes.contains(&Index::Height));
            }

            // cumulative should have multiple indexes merged
            if let Some(TreeNode::Leaf(l)) = map.get("cumulative") {
                assert_eq!(l.name(), "metric_cumulative");
                let indexes = l.indexes();
                // Height from height_cumulative, DateIndex from dateindex, Week/Month from dates, DifficultyEpoch
                assert!(indexes.contains(&Index::Height));
                assert!(indexes.contains(&Index::DateIndex));
                assert!(indexes.contains(&Index::WeekIndex));
                assert!(indexes.contains(&Index::MonthIndex));
                assert!(indexes.contains(&Index::DifficultyEpoch));
            }
        }
        _ => panic!("Expected branch"),
    }
...
#[test]
fn computed_block_full_inner_structure() {
    // Test the inner structure of a single ComputedBlockFull
    let value = make_computed_block_full("metric");
    let tree = value.to_tree_node();
    ...
    match &tree {
        TreeNode::Branch(map) => {
            ...
            // Expected flat structure after all merges:
            // base, cumulative, avg, sum, min, max, average, percentiles
            // Note: dateindex and difficultyepoch are merged in, not separate branches

            // base from height with wrap="base"
            assert!(map.contains_key("base"));
            if let Some(TreeNode::Leaf(l)) = map.get("base") {
                assert_eq!(l.name(), "metric");
                assert!(l.indexes().contains(&Index::Height));
            }

            // cumulative from height_cumulative + dateindex.sum_cum.cumulative + dates + difficultyepoch
            assert!(map.contains_key("cumulative"));
            if let Some(TreeNode::Leaf(l)) = map.get("cumulative") {
                assert_eq!(l.name(), "metric_cumulative");
                let indexes = l.indexes();
                // Should have merged: Height, DateIndex, WeekIndex, MonthIndex, DifficultyEpoch
                assert!(indexes.contains(&Index::Height), "Missing Height");
                assert!(indexes.contains(&Index::DateIndex), "Missing DateIndex");
                assert!(indexes.contains(&Index::WeekIndex), "Missing WeekIndex");
                assert!(indexes.contains(&Index::MonthIndex), "Missing MonthIndex");
                assert!(indexes.contains(&Index::DifficultyEpoch), "Missing DifficultyEpoch");
            }

            // avg, sum, min, max merged from dates + difficultyepoch LazyFull
            assert!(map.contains_key("avg"), "Missing avg");
            assert!(map.contains_key("sum"), "Missing sum");
            assert!(map.contains_key("min"), "Missing min");
            assert!(map.contains_key("max"), "Missing max");

            // average from dateindex.distribution.average
            assert!(map.contains_key("average"), "Missing average");

            // percentiles stays grouped
            assert!(map.contains_key("percentiles"), "Missing percentiles");

            // dateindex and difficultyepoch should NOT be separate branches
            assert!(!map.contains_key("dateindex"), "dateindex should be merged in");
            assert!(!map.contains_key("difficultyepoch"), "difficultyepoch should be merged in");
            assert!(!map.contains_key("minmax"), "minmax should be flattened");
        }
        _ => panic!("Expected branch"),
    }
}
