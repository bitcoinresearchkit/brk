...
use vecdb::{
    AnyStoredVec, BytesVec, CacheBudget, ColumnId, ColumnarVec, Database, Ident, ImportOptions,
...
fn column_chunks_borrow_cached_allocations_and_preserve_denial_and_rewrites() {
    for limit in [40_000 * 16, 0] {
        let directory = tempdir().unwrap();
        let db = Database::open(directory.path()).unwrap();
        let budget = Box::leak(Box::new(CacheBudget::new(limit)));
        let mut source = ColumnarVec::<BytesVec<usize, u64>, Column>::import_with(
            ImportOptions::new(&db, "columns", Version::ONE).with_cache_budget(budget),
        )
        .unwrap();
        for i in 0..40_000u64 {
            source.push([i, 7]);
        ...
        source.write().unwrap();
        let columns = source.read_only_clone();
        let snapshot = columns.column_snapshot(Column::First);
